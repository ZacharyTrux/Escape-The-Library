Shader "FX/ProceduralPortalURP"
{
    Properties
    {
        ...
        [Header(Colors)]
        [HDR]_OuterColor("Outer Color", Color) = (0.65, 0.95, 1.0, 1)
        [HDR]_OuterGlowColor("Outer Glow Color", Color) = (0.45, 0.85, 1.0, 1)
        [HDR]_InnerFrameColor("Inner Frame Color", Color) = (0.75, 1.0, 1.0, 1)
        [HDR]_InnerColorA("Inner Color A", Color) = (0.03, 0.12, 0.22, 1)
        [HDR]_InnerColorB("Inner Color B", Color) = (0.08, 0.55, 1.0, 1)
        ...
        [Header(Sparks)]
        [HDR]_SparkColor("Spark Color", Color) = (0.9, 1.0, 1.0, 1)
    ...
    SubShader
    {
        ...
        float Hash11(float p) { return frac(sin(p * 127.1) * 43758.5453123); }
        ...
        float SparkField(float a01, float r, float t, float outerR, float haloR)
        {
            ...
            float id = floor(x); float f = frac(x);
            float rndA = Hash11(id + 2.17); float rndB = Hash11(id + 7.31); float rndC = Hash11(id + 13.9);
            ...
            float da = abs(f - rndA); da = min(da, 1.0 - da);
            float dr = abs(r - centerR);
            float d = sqrt(da * da + dr * dr);
            float s = smoothstep(_SparkSize, 0.0, d);
            float flicker = 0.45 + 0.55 * sin((t + rndB) * 12.0 + rndA * 6.28);
        ...
        void EvaluatePortal(float2 uv, out float3 centerCol, out float centerA, out float3 glowCol, out float glowA)
        {
            ...
            float halo = (1.0 - smoothstep(outerR, haloR, r)) * clipMask;
            float n1 = Fbm2(p * _NoiseScale + t * _NoiseSpeed);
            float n2 = Fbm2(p * (_NoiseScale * 1.37) + t * (_NoiseSpeed * 1.15) + 19.31);
            float2 q = p + (float2(n1, n2) - 0.5) * _Distortion;
            float rq = length(q); float aq = atan2(q.y, q.x);

            aq += (1.0 - saturate(rq)) * _SwirlStrength * (0.65 + 0.35 * sin(t * _SwirlSpeed + rq * 6.0 + n1 * 2.0));
            float2 w = float2(cos(aq), sin(aq)) * rq;
            float bands = 0.5 + 0.5 * sin(aq * _BandCount + t * _BandSpeed + Fbm2(w * _NoiseScale + t * _NoiseSpeed) * 6.0);
            
            centerCol = saturate(_HoleColor.rgb + lerp(_InnerColorA.rgb, _InnerColorB.rgb, saturate(n1)) * _InnerIntensity * (0.35 + 0.85 * bands) * (0.35 + 0.65 * pow(saturate(1.0 - rq), max(_CenterFade, 1e-3))));
            centerA = saturate(innerArea) * _Opacity;

            float flicker = 1.0 + (edgeN - 0.5) * _EdgeFlicker;
            float3 outerCol = (_OuterColor.rgb * 0.75 + _OuterGlowColor.rgb * 0.55) * pow(saturate(outerFrame), 0.35) * (_OuterIntensity * flicker);
            float3 iFrameCol = _InnerFrameColor.rgb * pow(saturate(innerFrame), 0.45) * (_InnerFrameIntensity * (0.9 + 0.1 * sin(t * 7.5 + n1 * 4.0)));
            float3 hCol = _OuterGlowColor.rgb * pow(saturate(halo), 1.35) * _HaloIntensity;
            float3 sCol = _SparkColor.rgb * SparkField(a01, r, t, outerR, haloR) * _SparkIntensity;

            glowCol = saturate(outerCol + iFrameCol + hCol + sCol);
        ...
        Varyings vert(Attributes v)
        {
            ...
            float expand = max(_MeshExpand, 1.0) * max(1.0, haloR * max(_Size.x, _Size.y));
        ...
        Pass
        {
            ...
            half4 frag(Varyings i) : SV_Target {
                float3 c; float a; float3 g; float ga;
